...
class Speck64(object):
    ...
    def expand_key(self, k, t, version=96):
    ...
    @classmethod
    def check_testvector(cls, version=96):
        cipher = cls(alpha=8, beta=3)

        if version == 96:
            key = (0x20181110,0x09080100,0x19181110)
            pt = (0x6574694c, 0x6674694a)
            ks = cipher.expand_key(key, 26, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0xb78e92bc, 0xdcc5d1d4):
                flag = 1
            else:
                flag = 0
        elif version == 128:
            key = (0x20181110,0x09080100,0x19181110,0x090801aa)
            pt = (0x6574694c, 0x6674694a)
            ks = cipher.expand_key(key, 27, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0x3afb2752, 0x2c3607aa):
                flag = 1
            else:
                flag = 0
        if flag == 1:
    ...
    def make_train_data(self, n, nr, diff, data_form='only_diff', key_bit_length=96):
        ...
        c0r = self.ror((c0l ^ c0r), self.beta)  # Calulating back
    ...
    def make_train_data_multidiff(self, n, nr, diffs, data_form='only_diff', version=96):
        ...
        Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
        mk = np.frombuffer(urandom(n * 4 * mk_num), dtype=np.uint32).reshape((mk_num, n))
        p = [np.frombuffer(urandom(4 * n), dtype=np.uint32) for _ in range(2)]
        paired_p = []
        for diff in diffs:
...
if __name__ == "__main__":
    Speck64.check_testvector(version=96)
    Speck64.check_testvector(version=128)