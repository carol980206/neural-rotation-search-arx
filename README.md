...
- An efficient approximate evaluation function `$\hat{f}$`
- A greedy-with-exploration optimizer for large parameter spaces
- A joint neural distinguisher ($\mathcal{JND}$) with difference sensitivity test
...
5. A neural distinguisher $\mathcal{ND}(E^r_θ, Δ*)$ is trained.
...
├── neural_evaluator.py          # $\hat{f}$ implementations: $\mathcal{ND}$ and $\mathcal{JND}$ training pipelines
