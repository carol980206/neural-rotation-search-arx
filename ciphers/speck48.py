...
class Speck48(object):
    ...
    def expand_key(self, k, t, version=72):
    ...
    @classmethod
    def check_testvector(cls, version=72):
        cipher = cls(alpha=8, beta=3)

        if version == 72:
            key = (0x1f1e1d,0x1c1b1a,0x191865)
            pt = (0x65736f, 0x687420)
            ks = cipher.expand_key(key, 22, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0xb9c2c2, 0x6f1de2):
                flag = 1
            else:
                flag = 0
        elif version == 96:
            key = (0x1f1e1d,0x1c1b1a,0x191865,0x736f68)
            pt = (0x65736f, 0x687420)
            ks = cipher.expand_key(key, 23, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0x85c71c, 0x2185dd):
                flag = 1
            else:
                flag = 0
        if flag == 1:
    ...
    def make_train_data(self, n, nr, diff, data_form='only_diff', key_bit_length=72):
        Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
        mk_num = key_bit_length // self.WORD_SIZE
        mk = np.frombuffer(urandom(n * 4 * mk_num), dtype=np.uint32).reshape((mk_num, n))
        mk = mk & ((1 << self.WORD_SIZE) - 1)
        rk = self.expand_key(mk, nr, key_bit_length)
        p0l = np.frombuffer(urandom(4 * n), dtype=np.uint32)
        p0l = p0l & ((1 << self.WORD_SIZE) - 1)
        p0r = np.frombuffer(urandom(4 * n), dtype=np.uint32)
        p0r = p0r & ((1 << self.WORD_SIZE) - 1)
        p1l = p0l ^ diff[0]; p1r = p0r ^ diff[1]
        rand_sample_pos = (Y == 0)
        num_rand_samples = np.sum(rand_sample_pos)
        p1l[rand_sample_pos] = np.frombuffer(urandom(4 * num_rand_samples), dtype=np.uint32)
        p1l[rand_sample_pos] = p1l[rand_sample_pos] & ((1 << self.WORD_SIZE) - 1)
        p1r[rand_sample_pos] = np.frombuffer(urandom(4 * num_rand_samples), dtype=np.uint32)
        p1r[rand_sample_pos] = p1r[rand_sample_pos] & ((1 << self.WORD_SIZE) - 1)
    ...
    def make_train_data_multidiff(self, n, nr, diffs, data_form='only_diff', version=96):
        mk_num = version // self.WORD_SIZE
        Y = np.frombuffer(urandom(n), dtype=np.uint8) & 1
        mk = np.frombuffer(urandom(n * 4 * mk_num), dtype=np.uint32).reshape((mk_num, n))
        mk = mk & ((1 << self.WORD_SIZE) - 1)
        p = [(np.frombuffer(urandom(4 * n), dtype=np.uint32)) & ((1 << self.WORD_SIZE) - 1) for _ in range(2)]
        paired_p = []
        for diff in diffs:
        ...
        for i in range(len(diffs)):
            paired_p[i][0][negative_pos] = np.frombuffer(urandom(4 * num_rand_samples), dtype=np.uint32)
            paired_p[i][0][negative_pos] = paired_p[i][0][negative_pos] & ((1 << self.WORD_SIZE) - 1)
            paired_p[i][1][negative_pos] = np.frombuffer(urandom(4 * num_rand_samples), dtype=np.uint32)
            paired_p[i][1][negative_pos] = paired_p[i][1][negative_pos] & ((1 << self.WORD_SIZE) - 1)
...
if __name__ == "__main__":
    Speck48.check_testvector(version=72)
    Speck48.check_testvector(version=96)