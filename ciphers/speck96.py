...
class Speck96(object):
    ...
    def check_testvector(cls, version=96):
        ...
        if version == 96:
            key = (0x201811100908, 0x010019181110)
            pt = (0x201811100908, 0x010019181110)
            ks = cipher.expand_key(key, 28, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0x4247290e93bf, 0x327187a984ee):
                flag = 1
            else:
                flag = 0
        elif version == 144:
            key = (0x1f1e1d1c1b1a, 0x201811100908, 0x010019181110)
            pt = (0x201811100908, 0x010019181110)
            ks = cipher.expand_key(key, 29, version=version)
            ct = cipher.encrypt(pt, ks)
            if ct == (0x8617a1fbe980, 0x84de12d02d73):
                flag = 1
            else:
                flag = 0
        if flag == 1:
    ...
    def make_train_data(self, n, nr, diff, data_form='only_diff', key_bit_length=96):
        ...
        p1l = p1l & ((1 << self.WORD_SIZE) - 1)
        p1r = p1r & ((1 << self.WORD_SIZE) - 1)
    ...
    def make_train_data_multidiff(self, n, nr, diffs, data_form='only_diff', version=96):
        ...
        mk = np.frombuffer(urandom(n * 8 * mk_num), dtype=np.uint64).reshape((mk_num, n))
        mk = mk & ((1 << self.WORD_SIZE) - 1)
        p = [(np.frombuffer(urandom(8 * n), dtype=np.uint64)) & ((1 << self.WORD_SIZE) - 1) for _ in range(2)]
        paired_p = []
        for diff in diffs:
...
if __name__ == "__main__":
    Speck96.check_testvector(version=96)
    Speck96.check_testvector(version=144)